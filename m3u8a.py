...
def deal_m3u8(filename):
    ...
    print(pardir)
    key = None
    # 如果目录下有0.key,说明有加密，进行加密处理
    if os.path.exists(os.path.join(pardir, '0.key')):
        print('检测到加密key,准备读取')
        with open(os.path.join(pardir, '0.key'),"rb") as file:
            key = file.read()
    ...
    with open(os.path.join(os.getcwd(), out_file), mode='wb+') as file:
        ...
        print("=" * 10, out_file, "=" * 10)
    print(f'{out_file}写入完成!!!')
    print('结束')
